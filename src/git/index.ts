...
export async function discover(url: string, fetchOptions: RequestInit = {}, service: string = "git-upload-pack"): Promise<IGitInfoRefs> {
...
export function connect(url: string, fetchOptions: RequestInit = {}, service: string = "git-upload-pack"): Promise<Stream> {
	...
	return new Promise(async (resolve, reject) => {
		try {
			...
			listpack(response.packfile, ({ data, type, reference, offset, num }) => {
				...
				const hash = shasum(Buffer.concat([
					Buffer.from(`${typeName} ${data.byteLength.toString()}\0`),
					Buffer.from(data)
				]))
